...
const userUpdateService = async ({ id, name, email, cpf, phone, birthDate, description, typeUser, password, cep, state, city, road, numberHouse, complement }: IUserUpdate) => {
    ...
    if (newUser.typeUser != "buyer" && newUser.typeUser != "advertiser") {
        throw new AppError(400, `the ${newUser.typeUser} user type cannot be registered`)
    }
